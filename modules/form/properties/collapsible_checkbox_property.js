...
import propertySpecExtender from 'react-utility-belt/es6/form/utils/property_spec_extender';
import Checkbox from 'react-utility-belt/es6/ui/checkbox';
import { PropStuffer } from 'react-utility-belt/es6/utils/react';
...
const PropertyCheckboxHeading = CssModules(({ disabled, expanded, handleExpandToggle, label, name }) => (
    <button onClick={handleExpandToggle} styleName="checkbox-header" type="button">
        <Checkbox
            checked={expanded}
            disabled={disabled}
            label={label}
